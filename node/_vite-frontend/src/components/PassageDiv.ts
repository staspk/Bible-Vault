...
export class PassageDiv {
    ...
    static Generate(book:Book, chapter:number, data:IChapterResponse) {
        ...
        PassageDiv.alignVerses(book, chapter);
    }

    /**  Helper function to PassageDiv.Generate(). Aligns rows so all verses from all translations start at the same y position.  */
    static alignVerses(book:Book, chapter:number) {
