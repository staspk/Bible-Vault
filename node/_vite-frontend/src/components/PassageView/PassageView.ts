...
import { ContentView, TRANSLATIONS } from "../../..";
...
export class PassageView {
    ...
    static async Render(search:BibleSearch, onto:HTMLElement=ContentView.PlaceHolder()) {
        const queryString = BibleApi.From(search, TRANSLATIONS).queryString();
        Document.Title(search.toString(), queryString);
        SearchInput.Placeholder(search.toString());

        BibleApi.Fetch(queryString).then(data => {
            if(!data) return;

            let [view1_translations = 0, view2_translations] = translations_per_view(TRANSLATIONS.length);

            this.view1 = this.generateView(search.chapter, view1_translations, IChapter.range(0, view1_translations, data));
            if(this.view2) delete this.view2;
            if(view2_translations > 0)
                this.view2 = this.generateView(search.chapter, view2_translations, IChapter.range(view1_translations, (view1_translations+view2_translations), data));
